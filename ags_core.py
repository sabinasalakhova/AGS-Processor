...
def AGS4_to_dict(filepath_or_buffer, encoding='utf-8'):
    ...
    filepath_or_buffer : File path (str, pathlib.Path), or StringIO.
        Path to AGS4 file or any object with a read() method
    
    Returns
    -------
    data : dict
        Python dictionary populated with data from the AGS4 file
    headings : dict
...
def concat_ags_files(uploaded_files, giu_number):
    """
    Concatenate multiple AGS files into Excel format.
    ...
    group_list = ['PROJ', 'HOLE', 'GEOL', 'WETH', 'CORE', 'FRAC', 'DETL', 'ISPT', 
                  'SAMP', 'LEGD', 'DISC', 'PTIM', 'HORN', 'ABBR', 'CDIA', 'CLSS', 
                  'CNMT', 'CODE', 'DICT', 'DPRB', 'DPRG', 'DREM', 'FLSH', 'GRAD', 
                  'HDIA', 'IDEN', 'IPRM', 'IVAN', 'POBS', 'PREF', 'PRTD', 'PRTG', 
                  'PRTL', 'ROCK', 'SHBG', 'SHBT', 'TRIG', 'TRIX', 'UNIT']
    
    # Initialize empty dataframes for all groups
